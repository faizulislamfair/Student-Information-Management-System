...
$db_datas = mysqli_query($link, "SELECT * FROM `student_marks4` WHERE `Roll` = '$roll'");
$db_row = mysqli_fetch_assoc($db_datas);


if(isset($_POST['ct4update'])){
    ...
    $query = "UPDATE `student_marks4` SET `Electrical_Machines`= '$elec' ,`Data_Structure_&_Algorithms`= '$data' ,`Digital_Techniques`= '$tech'  ,`Mathematics`= '$math'  ,`Chemistry`= '$chem'  WHERE `Roll` = '$roll'";

    $result = mysqli_query($link, $query);
  

  if($result){
    // header('location: ct4display.php');
  }
...
<div style="margin-top:15px; height:35px;" class="form-group">
          <input type="submit" value="Update" name="ct4update" class="btn btn-primary">
        </div>
...
<a href="ct4display.php"><button style="margin-left:1080px; transform: translateY(-125px);" class="btn btn-primary">Back</button></a>
